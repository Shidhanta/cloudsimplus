...
public final class CloudletSchedulerTest extends CloudletSchedulerTimeShared{
    ...
    public double getStdDev(){
        double std = 0.0;
        double mean = getMeanBurstTime();
        for(CloudletExecution c: getCloudletWaitingList()){
            double x = getRemainingBurstTime(c);
            std += Math.pow((x-mean),2);
        }
        return Math.sqrt(std/getCloudletWaitingList().size());
    }
    /**
     *Pearson' s Correlation Coefficient to determine the skewness of the current Cloudlet Group
     */
    public double getPearsonsCoeff(){
        double mean = 0.0;
        double median = 0.0;
        double p  = 0.0;
        int n = getCloudletWaitingList().size();
    ...
    /**
     * Defines and returns time slice of a cloudlet on the basis of the Pearsons Correlation coefficient
     * @param cloudlet
     * @return
     */
    private double computeCloudletTimeSlice(final CloudletExecution cloudlet){
        double timeSlice = 0.0;
        double p = getPearsonsCoeff();
        double mean = 0.0;
        if(findCloudletInList(cloudlet.getCloudlet(),getCloudletWaitingList()).isPresent()){
            ...
            mean = getMeanBurstTime(cloudlet);
        }
    ...
    private List<CloudletExecution> cloudletsFinished(){
        ...
        final Predicate<CloudletExecution> noRemainingBurstTime = cle -> getRemainingBurstTime(cle) <= 0.0; //|| cle.getRemainingCloudletLength()<=0.0;
